 ...
 * <p>
 * Property {@link PropertyKeys#ASYNC_LOGGER_QUEUE_FULL_POLICY} controls the routing behaviour. If this property is
 ...
 * This can be adjusted with property {@link PropertyKeys#ASYNC_LOGGER_DISCARD_THRESHOLD} (name of the level at
 * which to start
 * discarding).
 * </p> <p>
...
public class AsyncQueueFullPolicyFactory {
    ...
    public static AsyncQueueFullPolicy create() {
        return create(PropertyEnvironment.getGlobal().getProperty(PropertyKeys.AsyncQueueFullPolicy.class));
    }

    static AsyncQueueFullPolicy create(final PropertyKeys.AsyncQueueFullPolicy config) {
    ...
    private static AsyncQueueFullPolicy createDiscardingAsyncQueueFullPolicy(
            final PropertyKeys.AsyncQueueFullPolicy config) {
